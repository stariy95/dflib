 ...
 * accepts an argument of exactly that type. An untyped argument, such as a bare column reference, is not accepted
 * and has to be cast by the caller ({@code castAsInt(a)}). A parameter declared as {@code Exp<?>} or
 * {@code Exp<T>} accepts an argument of any type;</li>
 ...
 * varargs, then the one with fewer arguments passed to {@code Exp<?>} parameters wins;</li>
